...
export const getUsersIds = async() => {
    ...
    return new Promise((resolve, reject) => {
        client.query("select id, role from users", (err, res) => {
            if(!err){
                resolve(res.rows)
            }else{
...
export const updateDocApplicantOrManagerSignature = async (id, payload, managerOrApplicantId, isManager = false) => {
    ...
    try {     
        ...
        let queryStr = isManager ? 
            `UPDATE rda 
             SET bucketref = $3, managersignature = $4, managerpublickey = $5` :
            `UPDATE rda 
             SET bucketref = $3, applicantsignature = $4, applicantpublickey = $5`;

        queryStr += where     
        // Log param length for debugging if needed
        console.log(params.length, "Number of parameters");
...
// export const getRdaById = async(id, payload) => {
//     // console.log(userData,"userDatauserDatauserDatauserData");
//     return new Promise(async(resolve, reject) => {
//         const client = await pool.connect()
//         try {     
//             const params = Object.keys(payload).map(field => 
//                 dbHashedFields.some(f => f === field ? Buffer.from(payload[field], "utf-8") : payload[field]))
//             const res = await client.query(`UPDATE users SET bucketref = $2, managersignature = $3, managerpublickey = $4 WHERE id = $1`, [id, ...params])//, (err, res) => {//${sets.join(" ")}
//             cLog("Correctly stored manager signature RDA with id " + id)
//             resolve(res.rowCount)
//         } catch (error) {
//             console.log(error,"error");
//             cLog(error?.message ?? error, "error")
